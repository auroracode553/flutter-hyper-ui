import 'package:flutter/material.dart';

/// Doc UI 默认色彩体系。
class DocUiColors {
  const DocUiColors._();

  static const Color background = Color(0xFFF8F9FB);
  static const Color foreground = Color(0xFF1A1D26);
  static const Color card = Color(0xFFFFFFFF);
  static const Color cardForeground = Color(0xFF1A1D26);
  static const Color primary = Color(0xFF3B82F6);
  static const Color primaryForeground = Color(0xFFFFFFFF);
  static const Color muted = Color(0xFFF1F3F5);
  static const Color mutedForeground = Color(0xFF6B7280);
  static const Color border = Color(0xFFE5E7EB);
  static const Color input = Color(0xFFE5E7EB);
  static const Color selectionBackground = Color(0xFFEFF6FF);

  static const Color darkBackground = Color(0xFF111318);
  static const Color darkForeground = Color(0xFFF3F4F6);
  static const Color darkCard = Color(0xFF1B1F27);
  static const Color darkMuted = Color(0xFF272C35);
  static const Color darkMutedForeground = Color(0xFFB8C0CC);
  static const Color darkBorder = Color(0xFF343B47);

  static const Color success = Color(0xFF22C55E);
  static const Color warning = Color(0xFFF59E0B);
  static const Color error = Color(0xFFEF4444);
  static const Color info = Color(0xFF3B82F6);
}
