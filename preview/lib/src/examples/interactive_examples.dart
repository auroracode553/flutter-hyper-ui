...
class _OverlayExampleState extends State<OverlayExample> {
  ...
  Widget build(BuildContext context) => HySpace(
    ...
    children: [
        ...
        child: HyWrap(
          children: [
            HyButton.tonal(
              label: '确认弹窗',
              onPressed: () async {
                final result = await HyDialog.confirm(
                  context,
                  title: '保存更改？',
                  message: '新的偏好将立即生效。',
                );
                if (context.mounted && result == true) {
                  HyToast.show(context, '已保存');
                }
              },
            ),
            HyButton.tonal(
            ...
            HyButton.tonal(
              label: '操作菜单',
              onPressed: () async {
                final result = await HyActionSheet.show(
                  context,
                  title: '照片操作',
                  actions: const [
                    HyAction(
                ...
                if (context.mounted && result != null) {
                  HyToast.show(context, '选择：$result');
                }
              },
            ),
            HyButton.tonal(
            ...
            HyPopupMenu(
              actions: const [
                HyAction(value: '编辑', label: '编辑'),
                HyAction(value: '分享', label: '分享'),
              ],
      ...
      const HyCard(
        title: '局部加载',
        child: Center(child: HyLoading(label: '正在同步')),
      ),
    ],
  );
}
...
class _BusinessExampleState extends State<BusinessExample> {
  ...
  Widget build(BuildContext context) => HySpace(
    ...
    children: [
      ...
      if (_notice)
        HyNoticeBar(
          message: '欢迎体验 Hy UI 柔光玻璃组件库。所有组件共享主题、间距与视觉规范，让每一页都自然一致。',
...
class _ListInteractionExampleState extends State<ListInteractionExample> {
  ...
  Widget build(BuildContext context) => HyCard(
    ...
    child: HySpace(
      ...
      children: [
        HyButton.ghost(
          label: _failNext ? '下次请求将展示失败重试' : '模拟下次加载失败',
                ...
                slivers: [
                  const HySticky(
                    child: Padding(
                      ...
                      child: Text('最近动态 · 吸顶标题'),
                    ),
                  ),
                  SliverList(
                    delegate: SliverChildBuilderDelegate(
                      (context, index) => HyListTile(
                        grouped: true,
                        title: '动态 ${index + 1}',
                        subtitle: '向下滚动加载更多',
                        leading: HyAvatar(text: '${index + 1}'),
                        onTap: () => HyToast.show(context, '动态 ${index + 1}'),
                      ),
                      childCount: _count,
                    ),
                  ),
